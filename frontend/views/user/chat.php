<div class="row-fluid" style="margin-top: 100px"></div>
<?php
...
function xmpp_connect($options, $access_token) {
    ...
    print "32321111111111111111111<br>";
    if (!find_xmpp($fp, 'MECHANISM', 'X-FACEBOOK-PLATFORM')) {
        return false;
    }
...
function get_access_token($app_id, $app_secret, $my_url){
    ...
    $code = FacebookUtil::getInstance()->getSavedUserToken(Yii::app()->user->model->id);
...
function _main() {
    ...
    $my_url = "http://dev.itake";
    $uid = Yii::app()->user->model->fbId;
    $access_token = FacebookUtil::getInstance()->getSavedUserToken(Yii::app()->user->model->id);
